...
def run(interval: int, once: bool, min_exchanges: int, no_export: bool, no_score: bool):
    ...
    while True:
        ...
        next_event = _next_joint_event_ms(records) if not no_score else None
...
def _next_joint_event_ms(records: dict) -> Optional[int]:
    ...
    have aligned funding events (within ALIGNMENT_TOLERANCE_SECONDS).
    Returns None if no aligned events are found in the current data.
    """
    now_ms = time.time() * 1000
